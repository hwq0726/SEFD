...
parser.add_argument('--output_file', default="data/gpt2_xl.jsonl_pp")
parser.add_argument('--para_src', default="paraphrase_outputs")
parser.add_argument('--para_dst', default="paraphrase_outputs_2")
...
if args.num_shards > 1:
    ...
    output_file = args.output_file + f"_pp{args.para_dst[-1]}"
...
for idx, dd in tqdm.tqdm(enumerate(data), total=len(data)):
    if idx < num_output_points:
        continue
    # tokenize prefix
    if args.refresh or args.para_dst not in dd:
        paraphrase_outputs_k = {}
        for lex, order in [(40, 40)]:
            if isinstance(dd[args.para_src]['lex_40_order_40']['output'], str):
                input_gen = dd[args.para_src]['lex_40_order_40']['output'].strip()
            else:
                input_gen = dd[args.para_src]['lex_40_order_40']['output'][0].strip()
            ...
            paraphrase_outputs_k[f"lex_{lex}_order_{order}"] = {
                "final_input": final_input_text,
                "output": [output_text.strip()],
                "lex": lex,
                "order": order
            }
        dd[args.para_dst] = paraphrase_outputs_k
