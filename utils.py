...
GPTZERO_API_KEYS = ['58654fc7baa748e2afbe89c6a11d6c28']
gptzero_idx = 0
...
def load_shared_args(parser):
    ...
    parser.add_argument('--paraphrase_type', default='lex_40_order_40', type=str)
...
def get_pool_score(x, detect, cache, cache_dir):
    """define a function to get the score of the pool."""
    score_list = []
    for i in x:
        score, cache1 = detect(i, cache)
...
def update_pool_random_both(pool, gens_list_score, gens_list_sim, sen, dec_score, sim_score):
    """define a function to update the pool."""
    temp = []
    pool_update = False
    for i in range(len(pool)):
        if gens_list_score[i] < dec_score and gens_list_sim[i] < sim_score:
            temp.append(i)
    if len(temp) == 0:
